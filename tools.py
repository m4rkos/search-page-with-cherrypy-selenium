...
class Tools:
    ...
    def test(self, some_string):
        return f"teste {some_string}"

    def mainContent(self, some_string):
    ...
    def mainImages(self, some_string):
        name = some_string
        idf = str(uuid.uuid4())
        if name != None:
    ...
    def downloadImages(self, url, t, name, idf):
        ...
        if name != '' :
            ...
            if t == 'DuckDuckGo':                
                ...
                for rs in res_items:
                    try:
                        urllib.request.urlretrieve(rs.get_attribute('src'), f"{images_path}/{img_name}_{number}.jpg")
                        number += 1                                                
                    finally:
                        pass                    
                    pass
                ...
                self.sqliteSaveResToWeb(data_to_render)
